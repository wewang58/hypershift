...
func InstallCiliumNetworkPolicies(t *testing.T, ctx context.Context, guestClient crclient.Client, hostedCluster *hyperv1.HostedCluster) {
	t.Run("InstallCiliumNetworkPolicies", func(t *testing.T) {
		if hostedCluster.Spec.Platform.Type != hyperv1.AzurePlatform {
			t.Skipf("test only supported on Azure platform, saw %s", hostedCluster.Spec.Platform.Type)
		}
		...
		if hostedCluster.Spec.Networking.NetworkType != hyperv1.Other {
			t.Skipf("test requires NetworkType 'Other' for Cilium, saw %s", hostedCluster.Spec.Networking.NetworkType)
		}
		...
		t.Run("InstallCilium", func(t *testing.T) {
			...
			podCIDR, hostPrefix := getCiliumNetworkConfig(hostedCluster)
			...
			if err != nil {
				if apierrors.IsAlreadyExists(err) {
					...
					err = guestClient.Get(ctx, crclient.ObjectKey{Name: "cilium", Namespace: "cilium"}, existingConfig)
			...
			g.Eventually(func() bool {
				...
				err := guestClient.Get(ctx, crclient.ObjectKey{Name: "cilium", Namespace: "cilium"}, ciliumConfig)
				if err != nil {
			...
			g.Eventually(func() bool {
				...
				err := guestClient.List(ctx, dsList, crclient.InNamespace("cilium"))
				if err != nil {
			...
			g.Eventually(func() bool {
				...
				// podList := &corev1.PodList{}
				// if err := guestClient.List(ctx, podList, crclient.InNamespace("cilium")); err == nil {
				// 	t.Log("Current pods in cilium namespace:")
				// 	for _, pod := range podList.Items {
				// 		t.Logf("  Pod %s: Phase=%s, Ready=%v", pod.Name, pod.Status.Phase, isPodReady(&pod))
				// 		if strings.HasPrefix(pod.Name, ciliumDaemonSet.Name) {
				// 			// Log container status if not ready
				// 			if !isPodReady(&pod) {
				// 				for _, cs := range pod.Status.ContainerStatuses {
				// 					if !cs.Ready {
				// 						t.Logf("    Container %s not ready: %+v", cs.Name, cs.State)
				// 					}
				// 				}
				// 			}
				// 		}
...
func ApplyYAMLFromURL(ctx context.Context, c crclient.Client, url string, defaultNamespace ...string) error {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("failed to download manifest from %s: HTTP %d", url, resp.StatusCode)
	}
...
// getCiliumNetworkConfig extracts pod CIDR and host prefix from HostedCluster
// In dual-stack environments, this function ensures we return the IPv4 network
func getCiliumNetworkConfig(hc *hyperv1.HostedCluster) (podCIDR string, hostPrefix int32) {
	podCIDR = "10.132.0.0/14"
	hostPrefix = 23

	for _, clusterNet := range hc.Spec.Networking.ClusterNetwork {
		cidrStr := clusterNet.CIDR.String()
		if strings.Contains(cidrStr, ".") {
			podCIDR = cidrStr
			// Only use clusterNet.HostPrefix if it's non-zero, otherwise keep the default
