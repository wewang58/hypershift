...
func TestCiliumConnectivity(t *testing.T) {
	...
	e2eutil.NewHypershiftTest(t, ctx, func(t *testing.T, g Gomega, mgtClient crclient.Client, hostedCluster *hyperv1.HostedCluster) {
		...
		t.Run("WaitForCiliumPodsReady", func(t *testing.T) {
			...
			t.Log("Waiting for Cilium pods to be ready")
			g.Eventually(func() bool {
				...
				if len(podList.Items) == 0 {
					t.Log("No Cilium pods found yet")
					return false
				}
				...
				return true
			}, e2eutil.CiliumPodsReadyTimeout, e2eutil.CiliumPodsReadyPollInterval).Should(BeTrue(), "all Cilium pods should be ready")

			t.Log("All Cilium pods are ready")
		})
